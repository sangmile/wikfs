...
리스트 함축 원칙은 세트와 딕셔너리까지 확장된다. 세트 함축은 대괄호 기호 대신 중괄호 기호를 사용하면 된다.

```python
my_set = {i for i in a_lst if a > 0}
```

딕셔너리 함축은 조금 더 복잡하다.

```python
>>> vals_lst = [ ('pi', 3.14), ('phi', 1.618) ]
>>> my_dict = { i[0]: i[1] for i in vals_lst }
>>> my_dict
{'pi': 3.14, 'phi': 1.618}
```

두 리스트를 하나의 딕셔너리로 합친 예이다.
...
grade\_dict = { key: val for key, val in zip\(keys, vals\) }
{% endhint %}

딕셔너리 함축은 키, 값을 바꾸는 경우 유용하다.

```python
idict = { v:k for k, v in phone_dict.items() }
```

데이터 딕셔너리의 items 메서드는 키와 값 쌍의 리스트를 생성한다.

## 리스트를 통한 인수 전달하기

파이썬 인수 값은 정확하게 말하자면 참조에 의해서 혹은 값에 의해서 전달되는 것이 아니다. 대신 파이썬 인수는 함수가 호출되는 순간의 인수 이름과 값이 연결된 데이터-딕셔너리 엔트리로 전달된다.

인수로 주어진 변수를 함수 안에서 수정할 수 없다는 의미다.

double\_it\(n\)

double\_it 함수가 인수 n으로 10이 전달되어 실행되었다고 하자. 함수는 키-값 쌍으로 n:10이 전달된다. 하지만 n에 새로운 값을 대입하면 마치 지역 변수를 변경한 것과 같이 함수 밖의 n에는 영향을 미치지 않는다. 대입하는 순간 n과 데이터의 연결 고리를 끊기 때문이다.

같은 방식으로 리스트를 함수의 인수로 전달하면 리스트의 일부 혹은 전부의 항목을 변경할 수 있다.

```python
def set_lst_vals(lst_arg):
    lst_arg[0] = 100
    lst_arg[1] = 200
    lst_arg[2] = 300

a_lst = [0, 0, 0]
set_lst_vals(a_lst) # [100, 200, 300]
```

다음 예시는 전달한 리스트를 변경할 수 없다.
...
a_lst = [0, 0, 0]
set_lst_vals(a_lst) # [0, 0, 0]
```

전달된 리스트 인수가 완전히 새로운 리스트로 재대입되었기 때문에 변수 lst\_arg와 기존 데이터 \[0, 0, 0\]의 연결 고리가 끊긴 것이다.

## 다차원 리스트

다차원 리스트는 일반적으로 행렬에서 사용한다.

```python
mat = [[10, 20, 30], [1, 2, 3], [1, 10, 30]]
```
...
이 코드는 \[0\] \* 100으로 100개의 항목을 가진 리스트를 생성한다. 하지만 \* 200은 동일한 데이터를 200번 반복하여 호출한다. 같은 행 참조 200개를 만드는 것이다.

200개의 행은 분리되지 않는다. 이는 얕은 복사다. 200개 행을 한 번에 추가하려면 for 루프를 사용해야 한다.
...
`행_번호`가 중요하지 않다면 언더스코어\(\_\)로 교체할 수 있다.

```python
mat2 = [ [0] * 25 for _ in range(30) ] # 25 X 30 행렬
```

다음 코드는 30X20X25 3차원 리스트다.

```python
mat = [[ [0] * 25 for _ in range(20) ]
                  for _ in range(30) ]
```
